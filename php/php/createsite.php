...
if (isset($_POST['originfqdn']) && isset($_POST['jspath'])) {
	...
	$gencert = shell_exec('/php/gen_cert.sh ' . $originfqdn);
	echo $gencert;
	...
	$configblock = $configblock . "  access_log syslog:server=elk:5140,facility=local7,tag=nginx,severity=info didformat_$jsfile;\n";
...
#	$configblock = $configblock . "    health_check;\n";
	...
	if (isset($_POST['logformatspec'])) {
		...
		$logformatblock = "log_format didformat_$jsfile '<190>#\$msec_no_decimal#\$remote_user#$originfqdn#\$remote_addr#12345#\$xff_ip#\$upstream_addr#\$host#\$request_method#\$uri#\$status#nocontentype#0#\$deviceA#\$deviceB#\$http_user_agent#0#empty#';\n";
	}
	...
	$configfile = "/etc/nginx/conf.d/sites/" . $originfqdn . ".conf";
	$logformatfile = "/etc/nginx/conf.d/logformat/" . $jsfile . ".conf";
	$jsfilefullpath = "/etc/nginx/conf.d/js/" . $jsfile . ".js";
	...
	echo "----------------------------------------------------\n";
	echo "writting below content to " . $jsfilefullpath;
	echo "\n";
	echo $jscontent;
	echo "\n\n";
	file_put_contents($jsfilefullpath , $jscontent);
	echo "----------------------------------------------------\n";

	echo "TODO: adding the below line to your /etc/hosts\n";
	echo "18.167.75.86  $originfqdn\n";
	echo "----------------------------------------------------\n\n";
